...
impl WeekTemperatures {
    pub fn new() -> Self {
        WeekTemperatures {
            week_temperatures: [None; 7],
        }
    }

    pub fn get_temperature(&self, day: Weekday) -> Option<i32> {
        let index: usize = WeekTemperatures::get_index_for_day(&day);
        self.week_temperatures[index]
    }

    pub fn set_temperature(&mut self, day: Weekday, temperature: i32) {
        let index: usize = WeekTemperatures::get_index_for_day(&day);
        self.week_temperatures[index] = Some(temperature);
    }

    fn get_index_for_day(day: &Weekday) -> usize {
        match day {
            Weekday::Monday => 0,
            Weekday::Tuesday => 1,
            Weekday::Wednesday => 2,
